...
//
// One screen rather than a list plus an editor, because a category is three short fields and
// there will never be many of them. Adding one is a row at the top of the list; editing one
// is typing in the row it already occupies. A separate page per shelf would be a navigation
// for a form that fits on a line.
//
// Deleting is offered without any check on whether the shelf is empty, and that is deliberate:
// `stories.category_id` is `on delete set null`, so the stories in it come off the shelf and
// go back to being unfiled. Nothing is lost, so nothing has to be warned about.

import { useState } from 'react';
import { Link } from 'react-router-dom';
import { Check, Plus } from 'lucide-react';
import type { StoryCategory } from '@georgian/shared/types';
...
/** "1 story", "4 stories". The "(s)" the other admin screens use reads badly on a count of one. */
...
export default function StoryCategoryList() {
  ...
  const unfiled = storyList.filter(story => !story.categoryId).length;

  const open = (category: StoryCategory | null) => {
    setEditing(category?.id ?? 'new');
    setDraft(
      category ? { name: category.name, nameNative: category.nameNative, note: category.note } : EMPTY,
    );
  };

  const save = async (id: string | null) => {
    const result = await run(() =>
      api.admin.saveStoryCategory({
        ...(id ? { id } : {}),
        lang: lang(),
  ...
  const remove = async (id: string) => {
    const result = await run(() => api.admin.deleteStoryCategory({ id }));
  ...
  return (
    <AdminPage>
      ...
      <AdminHeadRow>
        <div>
          <AdminTitle>Story categories</AdminTitle>
          <AdminSub>
            The headings the story index groups by. A story sits on one shelf or on none, and a story
            with none is listed under “Everything else”.
          </AdminSub>
        </div>
        {editing !== 'new' && (
      ...
      {categories.length === 0 && editing !== 'new' && (
        <AdminNote>
          None yet. Until there is one, the story index is a single list — which is what it has always
          been, and is fine for a dozen stories.
        </AdminNote>
      )}
            ...
            {editing === category.id ? (
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                >
                  ...
                  <AdminBadge>{stories(category.storyCount)}</AdminBadge>
                ...
                {confirming === category.id ? (
                  <span className="flex flex-wrap items-center gap-2.5 text-sm">
                    Delete it? The {stories(category.storyCount)} in it stay, unfiled.
                    <Button
      ...
      {unfiled > 0 && categories.length > 0 && (
        <AdminNote>
          {stories(unfiled)} {unfiled === 1 ? 'is' : 'are'} on no shelf, listed under “Everything else” —
          file {unfiled === 1 ? 'it' : 'them'} from <Link to="/admin/stories">the story list</Link>.
        </AdminNote>
      )}
    </AdminPage>
  );
