...
export interface CardRecord extends Review {
  ...
  /**
   * When this record was last written, epoch ms. The whole of the merge rule when an
   * account is syncing: for one card, the newer of the two copies wins.
   *
   * Deliberately not the same as `last`, which is when the card was last *answered*.
   * Undoing a misclick restores an older answer, so `last` goes backwards — and a merge
   * keyed on it would treat the undo as stale and quietly bring the misclick back.
   */
  updatedAt: number;
}
