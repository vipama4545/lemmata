  ...
  return (
    <div className="main-content">
      ...
      <header className="story-index-head">
        ...
        <p className="story-index-sub">
          Georgian short stories with every word linked back to the dictionary.
          Double-click a word while reading to see what it means.
        </p>
      </header>
      ...
      {stories.length === 0 ? (
        ...
        <div className="story-grid">
          {stories.map(story => {
            const linked = Math.round((story.stats.linkedTokens / story.stats.tokens) * 100);
            return (
              <Link key={story.id} to={`/stories/${story.id}`} className="story-card">
                <div className="story-card-head">
                  <h2 className="story-card-title">{story.title}</h2>
                  {story.level && (
                    <span className={`level-badge ${story.level.toLowerCase()}`}>{story.level}</span>
                  )}
                </div>
