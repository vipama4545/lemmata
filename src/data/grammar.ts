// Reference grammar. Unlike words.json and verbs.json this is hand-written rather than
// scraped, so it lives as a module: the two verb topics build their tables from the
// metadata already shipped in verbs.json instead of restating it here.
//
...
// The screeve and series tables restate what verbs.json already carries, so they are read
...
function pickExampleVerb(): Verb {
  ...
  for (const verb of verbData.verbs) {
    const filled = verbData.screeves.filter(s => verb.forms[s.key]?.['1sg']).length;
...
const verbGroups: GrammarTopic = {
  ...
  sections: [
    ...
    {
      groups: verbData.groups.map(group => ({
        label: group.label,
        name: group.name,
...
export function groupedGrammarTopics(): GrammarGroupWithTopics[] {
  return grammarGroups
    .map(group => ({
      ...group,
      topics: grammarTopics.filter(topic => topic.group === group.id),
    }))
