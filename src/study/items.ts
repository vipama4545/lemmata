...
/** verbId → the words.json id that claims it, for the 165 paradigms a headword covers. */
const claimedBy = new Map<string, string>();
for (const word of wordData.words) {
  if (word.verbId && !claimedBy.has(word.verbId)) claimedBy.set(word.verbId, word.id);
}
...
export const studyItems: StudyItem[] = [
  ...wordData.words.map(fromWord),
  ...verbData.verbs.filter(verb => !claimedBy.has(verb.id)).map(fromVerb),
];

export const itemsByKey = new Map(studyItems.map(item => [item.key, item]));
...
export const studyCategories = [
  ...wordData.categories.map(cat => ({ id: cat.id, name: cat.name })),
